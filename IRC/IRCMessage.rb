...
class IRCMessage
  ...
  def botcommand
    ...
    bc = message[/^\s*(#{@bot.user.nick}\s*[:>,]?\s*)?#{self.class::BotCommandPrefix}([\S]+)/i, 2] if message
  ...
  def tail
    tail = message[/^\s*(#{@bot.user.nick}\s*[:>,]?\s*)?#{self.class::BotCommandPrefix}([\S]+)\s*(.*)\s*/i, 3] || message if message
